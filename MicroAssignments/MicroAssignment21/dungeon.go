...
func (d *Dungeon) StampCorner(room Room, x, y int) bool {
	if room.Width > 4 && room.Height > 4 && rand.IntN(2) == 1 {
		if x == 0 && y == 0 { // DOES NOT WORK RIGHT
			d.PlaceBlock(WALL, x+room.PivotX+1, y+room.PivotY+1)
			return true
		} else if x == 0 && y == room.Height-1 { // DOES NOT WORK RIGHT
			d.PlaceBlock(WALL, x+room.PivotX+1, y+room.PivotY-1)
			return true
		} else if x == room.Width-1 && y == 0 { // works right
			d.PlaceBlock(WALL, x+room.PivotX-1, y+room.PivotY+1)
			return true
		} else if x == room.Width-1 && y == room.Height-1 { // works right
			d.PlaceBlock(WALL, x+room.PivotX-1, y+room.PivotY-1)
			return true
		}
	}
