...
func main() {
	...
	otherCreature := NewCreature(rl.NewVector2(100, 100))
	controlledCharacter := 0 // player by default
	...
	for !rl.WindowShouldClose() {
		...
		if rl.IsKeyPressed(rl.KeyT) {
			if controlledCharacter == 0 {
				controlledCharacter = 1
			} else {
		...
		if controlledCharacter == 0 {
			...
			otherCreature.MoveCreatureWithCamera(dir, camAngle)
			cam.Target = rl.NewVector2(float32(int32(otherCreature.pos.X)), float32(int32(otherCreature.pos.Y)))
		}
